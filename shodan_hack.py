...
def IPs_to_scan(IPs_input):
    ...
    with open(IPs_input) as IPs_to_scan:
        ...
        for IP in IPs_to_scan.readlines():
            ...
            try:
                print("\n\033[1;95m{}\033[1;00m\n".format(IP.strip()))
                json_object = requests.get("https://api.shodan.io/shodan/host/{0}?key={1}".format(IP.strip(), shodan_key)).json()
                count = 0
                ...
                with open("{0}/reports/shodan/{1}_full_report.json".format(os.getcwd(), IP.strip()), "w") as outfile:
            ...
            if os.path.exists("{0}/reports/shodan/{1}_full_report.json".format(os.getcwd(), IP.strip())):
